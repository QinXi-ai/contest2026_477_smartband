...
GCC_NM = BUILD4 / (
    "prebuilts/gcc/linux-x86_64/arm-none-eabi/bin/arm-none-eabi-gcc-nm"
)
READELF = BUILD4 / (
    "prebuilts/gcc/linux-x86_64/arm-none-eabi/bin/arm-none-eabi-readelf"
)
...
ROUTER_OBJECT = BUILD4 / (
    "packages/ai_agent/src/tools/mooncat_local_router.c."
    "data.openvela-contest-2026-gemini-s1-v1-build-20260830-04."
    "packages.ai_agent.o"
)
MODEL_OBJECT = BUILD4 / (
    "packages/ai_agent/src/tools/mooncat_intent_model.cc."
    "data.openvela-contest-2026-gemini-s1-v1-build-20260830-04."
    "packages.ai_agent.o"
)
...
def gcc_nm_defined_symbols(path: Path) -> dict[str, str]:
    completed = subprocess.run(
        [str(GCC_NM), "-a", "--defined-only", str(path)],
        check=True,
        capture_output=True,
        text=True,
        encoding="utf-8",
...
def dwarf_decl_lines(path: Path, names: tuple[str, ...]) -> dict[str, set[int]]:
    results = {name: set() for name in names}
    active: str | None = None
    remaining = 0
    process = subprocess.Popen(
        [str(READELF), "--debug-dump=info", str(path)],
    ...
    for line in process.stdout:
        if "DW_AT_name" in line:
            active = next((name for name in names if name in line), None)
            remaining = 12 if active else 0
            continue
        if active and "DW_AT_decl_line" in line:
            match = re.search(r"DW_AT_decl_line\s*:\s*(\d+)", line)
            if match:
                results[active].add(int(match.group(1)))
            active = None
            remaining = 0
            continue
        if remaining:
            ...
            if remaining == 0:
                active = None
    if process.wait() != 0:
        raise RuntimeError("readelf DWARF scan failed")
    return results
...
def main() -> None:
    ...
    for item in items:
        ...
        padding_zero = stored >= actual and region_is_zero(
            NEW_IMAGE, actual_end, stored - actual
        )
    ...
    required_anchors = (
        ...
        b"MOONCAT ACTIVE COACH",
        b"Quickapp mqueue listener started",
        b"[LOCAL AI",
        b"TFL3",
    )
    banned_anchors = (
    ...
    quickapp_symbols = sorted(
        name
        for name in symbols
        if re.fullmatch(r"quickapp_mq_listener_task(?:\.lto_priv\.\d+)?", name)
    )
    quickapp_symbol_pass = len(quickapp_symbols) == 1
    symbol_pass &= quickapp_symbol_pass
    symbol_lines.append(
        "LTO_AWARE\tquickapp_mq_listener_task\t"
        f"{','.join(quickapp_symbols) if quickapp_symbols else '-'}\t"
        f"{'PRESENT' if quickapp_symbol_pass else 'MISSING'}"
    )

    router_lto_symbols = gcc_nm_defined_symbols(ROUTER_OBJECT)
    model_lto_symbols = gcc_nm_defined_symbols(MODEL_OBJECT)
    dwarf_lines = dwarf_decl_lines(
        ELF,
        (
    ...
    lto_function_checks = (
        (
            "mooncat_local_router_handle",
            router_lto_symbols.get("mooncat_local_router_handle") == "T"
            and 348 in dwarf_lines["mooncat_local_router_handle"],
            "input_object_T_and_final_DWARF_line_348",
        ),
        (
            "mooncat_intent_model_predict",
            model_lto_symbols.get("mooncat_intent_model_predict") == "T"
            and 102 in dwarf_lines["mooncat_intent_model_predict"],
            "input_object_T_and_final_DWARF_line_102",
        ),
        (
            "quickapp_mq_listener_task",
            quickapp_symbol_pass and 225 in dwarf_lines["quickapp_mq_listener_task"],
            "final_LTO_symbol_and_DWARF_line_225",
        ),
    )
    for name, passed, evidence in lto_function_checks:
        symbol_pass &= passed
        symbol_lines.append(
            f"LTO_DEFINITION\t{name}\t{evidence}\t"
            f"{'PRESENT' if passed else 'MISSING'}"
        )
